...
* 1. [TiP - Why and How?](#tip---why-and-how?)
  * 1.1. [Initial Situation](#initial-situation)
    ...
    * 1.1.2. [Data Compession](#data-compession)
...
* 2. [ID Table Generation](#id-table-generation)
  * 2.1. [ID Table Generation Issue](#id-table-generation-issue)
...
##  1. <a name='tip---why-and-how?'></a>TiP - Why and How?

###  1.1. <a name='initial-situation'></a>Initial Situation

####  1.1.1. <a name='framing'></a>Framing
...
####  1.1.2. <a name='data-compession'></a>Data Compession
...
###  1.2. <a name='bytes-and-numbers'></a>Bytes and Numbers
...
###  1.3. <a name='the-tip-idea'></a>The TiP Idea

####  1.3.1. <a name='packing'></a>Packing
...
####  1.3.2. <a name='unpacking'></a>Unpacking

On the receiver side all bytes with MSBit=0 are identified as IDs and are replaced with the patterns they stay for. All bytes with MSBit=1 are carying the unreplacable bytes bits. These are ordered back to restore the unreplacable bytes which fill the wholes between the patterens.

##  2. <a name='id-table-generation'></a>ID Table Generation

* We create a bunch of test files with data similar to them we want to pack in the future.
...
* The same is done with all smaller pattern sizes, ergo 7, 6, 5, 4, 3, 2. Not interesting are 1-byte pattern, because their replacement by an ID gives no compression effect.
* The 127 most often occuring pattern are sorted by descending size and are used to create the file `idTable.c`.

###  2.1. <a name='id-table-generation-issue'></a>ID Table Generation Questions
...
* Also it could make sense to use the length of a pattern as weigth. If, for example a 5-bytes long pattern occurs 100 times and a 2-bytes long pattern exists 200 times in the sample data - which should get preceedence to get into the ID table?
...
##  3. <a name='improvement-ideas'></a>Improvement Ideas

###  3.1. <a name='reserve-some-ids-for-run-length-encoding'></a>Reserve some IDs for Run-Length Encoding

| ID sequence                    | Meaning        |
| ------------------------------ | -------------- |
| ID `FF` + `01`                 | reserved       |
| ID `FF` + `2...32`             | 2 to 32 zeroes |
| ID `FF` + `33...48`            | 2 to 16 FFs    |
| ID `FF` + `48...127` + `AA`!=0 | 4 to 80 `AA`s  |
| ID `FF` + `128...255`          | reserved       |




<!--
###  3.2. <a name='minimize-worst-case-size'></a>Minimize Worst-Case Size

* If data are containing no ID table data at all, they are getting bigger by the factor 8/7. Thats a result of treating the data in 8 bit units (bytes).
